...
import scala.math._
 ...
 class HMMAligner {
   ...
   private var trace_matches: ArrayBuffer[Char] = ArrayBuffer('\0')
   private var trace_inserts: ArrayBuffer[Char] = ArrayBuffer('\0')
   private var trace_deletes: ArrayBuffer[Char] = ArrayBuffer('\0')
   ...
   def alignSequences(_ref: String, _test: String, _test_quals: String): Boolean = {
     ...
     if (mat_size > old_mat_size && mat_size > 0) {
     ...
     } else if (old_mat_size == 0 && mat_size == 0) {
       matches = new ArrayBuffer[Double](1)
       inserts = new ArrayBuffer[Double](1)
       deletes = new ArrayBuffer[Double](1)
       trace_matches = new ArrayBuffer[Char](1)
       trace_inserts = new ArrayBuffer[Char](1)
       trace_deletes = new ArrayBuffer[Char](1)
     }
     ...
     if (mat_size > 0) {
       ...
       alignment_likelihood = max(matches(0), max(inserts(0), deletes(0)))
     }
 ...
 class ReadCallAssemblyPhaser extends ReadCall {
   ...
   */
  def phaseAssembly(region: Seq[ADAMRecord], kmer_graph: KmerGraph, ref: String): List[ADAMVariantContext] = {
    var ref_haplotype = new Haplotype(ref)
