...
class ReadExplorerSuite extends AvocadoFunSuite {
  ...
  sparkTest("observe a simple read") {
    ...
    assert(observations.forall(_.mismatchQScoreSum == 0))
  ...
  sparkTest("observe a read with soft clipping") {
    ...
    assert(observations.filter(_.pos.pos == 10L).length === 1)
    assert(observations.filter(_.pos.pos == 10L).head.distanceFromBeginningClippedRegion === Some(1))
    assert(observations.filter(_.pos.pos == 10L).head.distanceFromEndClippedRegion === Some(5))
    assert(observations.filter(_.pos.pos == 10L).head.allele === "A")
    assert(observations.filter(_.pos.pos == 11L).length === 1)
    assert(observations.filter(_.pos.pos == 11L).head.distanceFromBeginningClippedRegion === Some(2))
    assert(observations.filter(_.pos.pos == 11L).head.distanceFromEndClippedRegion === Some(4))
    assert(observations.filter(_.pos.pos == 11L).head.allele === "C")
    assert(observations.filter(_.pos.pos == 12L).length === 1)
    ...
    assert(observations.filter(_.pos.pos == 12L).head.allele === "T")
    assert(observations.filter(_.pos.pos == 13L).length === 1)
    assert(observations.filter(_.pos.pos == 13L).head.distanceFromBeginningClippedRegion === Some(4))
    assert(observations.filter(_.pos.pos == 13L).head.distanceFromEndClippedRegion === Some(2))
    assert(observations.filter(_.pos.pos == 13L).head.allele === "G")
    assert(observations.filter(_.pos.pos == 14L).length === 1)
    assert(observations.filter(_.pos.pos == 14L).head.distanceFromBeginningClippedRegion === Some(5))
    assert(observations.filter(_.pos.pos == 14L).head.distanceFromEndClippedRegion === Some(1))
    assert(observations.filter(_.pos.pos == 14L).head.allele === "A")
  }
