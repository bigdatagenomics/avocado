...
class ReadExplorerSuite extends AvocadoFunSuite {
  ...
  sparkTest("observe a simple read") {
    ...
    assert(observations.forall(_.distanceFromEndClippedRegion.isEmpty))
    assert(observations.forall(_.distanceFromBeginningClippedRegion.isEmpty))
  ...
  sparkTest("observe a read with a deletion") {
    ...
    assert(observations.filter(_.pos.pos == 11L).last.distanceToNearestReadDeletion === Some(0))
  ...
  sparkTest("observe a read with soft clipping") {
    ...
    assert(observations.forall(_.distanceToNearestReadDeletion.isEmpty))
    assert(observations.forall(_.distanceToNearestReadInsertion.isEmpty))
    assert(observations.forall(_.distanceFromEndClippedRegion.isDefined))
    assert(observations.forall(_.distanceFromBeginningClippedRegion.isDefined))
    assert(observations.filter(_.pos.pos == 10L).length === 1)
    assert(observations.filter(_.pos.pos == 10L).head.distanceFromEndClippedRegion === Some(1))
    assert(observations.filter(_.pos.pos == 10L).head.distanceFromBeginningClippedRegion === Some(5))
    assert(observations.filter(_.pos.pos == 10L).head.allele === "A")
    assert(observations.filter(_.pos.pos == 11L).length === 1)
    assert(observations.filter(_.pos.pos == 11L).head.distanceFromEndClippedRegion === Some(2))
    assert(observations.filter(_.pos.pos == 11L).head.distanceFromBeginningClippedRegion === Some(4))
    assert(observations.filter(_.pos.pos == 11L).head.allele === "C")
    assert(observations.filter(_.pos.pos == 12L).length === 1)
    assert(observations.filter(_.pos.pos == 12L).head.distanceFromEndClippedRegion === Some(3))
    assert(observations.filter(_.pos.pos == 12L).head.distanceFromBeginningClippedRegion === Some(3))
    assert(observations.filter(_.pos.pos == 12L).head.allele === "T")
    assert(observations.filter(_.pos.pos == 13L).length === 1)
    assert(observations.filter(_.pos.pos == 13L).head.distanceFromEndClippedRegion === Some(4))
    assert(observations.filter(_.pos.pos == 13L).head.distanceFromBeginningClippedRegion === Some(2))
    assert(observations.filter(_.pos.pos == 13L).head.allele === "G")
    assert(observations.filter(_.pos.pos == 14L).length === 1)
    assert(observations.filter(_.pos.pos == 14L).head.distanceFromEndClippedRegion === Some(5))
    assert(observations.filter(_.pos.pos == 14L).head.distanceFromBeginningClippedRegion === Some(1))
    assert(observations.filter(_.pos.pos == 14L).head.allele === "A")
  }
